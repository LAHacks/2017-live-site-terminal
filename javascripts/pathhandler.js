...
(function(root, $, _) {
  Josh.PathHandler = function(shell, config) {
    ...
    var _shell = shell;
    _shell.templates.not_found = _.template("<div><%=cmd%>: <%=path%>: No such directory or file</div>");
    _shell.templates.not_a_directory = _.template("<div><%=cmd%>: <%=path%>: Not a directory</div>");
        ...
        */
        if ('_META_TYPE' in node) {
          if (node['_META_TYPE'] !== 'dir') {
            return callback(_shell.templates.not_a_directory({cmd: 'cd', path: args[0]}));
          }
        }
        self.current = node;
        return callback();
